(function () {
  ...
  form.addEventListener("submit", async function (e) {
    ...
    const orderRaw = document.getElementById("stacked-order").value.trim();
    ...
    const userRows = result.rows.filter(function (r) {
      return !loginUserId || String(r["ユーザーID"] || "").trim() === loginUserId;
    });

    const orderNum = parseInt(orderRaw, 10);
    const duplicate = userRows.some(function (r) {
      return parseInt(String(r["表示順"] || ""), 10) === orderNum;
    });
    if (duplicate) {
      errorEl.textContent = "入力した表示順はすでに使用されています。別の値を入力してください。";
      return;
    }
